...
**Reciprocal-detector note (their observation, worth acting on).** EDH's own span check (`_span_shortfalls.json`, their #53) is shipped but **not armed** — today's run reports `members_checked: 66, members_with_expectation: 0`, no member having reached its 10-observation floor — and a shape signature cannot see span at all, since 96 and 192 hash identically. So for roughly the next week **Augur's gate is the better detector for this class on either side.** A refusal on point count is information upstream does not otherwise have: when the gate refuses a vintage, say so upstream rather than only logging it.
