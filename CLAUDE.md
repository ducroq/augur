...
- **Known weakness — no demonstrated skill over naive (opened 2026-09-06)**: horizon-matched over `t0 ∈ [2026-07-30, 2026-08-24]`, LGBM `p50` loses to "same clock hour, last day available at t0" at **every** horizon (1-24h 26.9 vs 23.3; 25-48h 33.9 vs 28.9; 49-72h 36.9 vs 32.7), and over 96 mixed-horizon eval days it wins on 42 — **44%**. This was invisible because every instrument here is relative: `eval_log` scores LGBM vs ARF, the registry scores variants vs production, DM compares two candidates. **Read every pre-2026-09-06 effect size with this in mind** — EXP-021's −20.3% QS is measured against a sub-naive incumbent. Verdict pre-committed on fresh vintages in `docs/hypothesis-log.md` [2026-09-06]; a rank-based metric (does it pick the cheap hours?) **was the untested alternative and is now closed — EXP-037, 2026-09-11: it ranks them WORSE than the carry (regret 7.99 vs 5.01 EUR/MWh, worse in 6 of 9 months), so the product lens is the harsher one and augur#29 is not narrowed by it.** Separately: the first ~24 forecast hours predict a day-ahead auction that clears ~4h *before* the run, and `api-client.js:226` already fetches those prices — a product change, not a model one, and not yet decided.
...
- **Model work 2026-08-25..30 — 13 experiments (EXP-018..EXP-033), nothing shipped, no production path touched.** The full record is **`docs/experiment-results.md`**, generated from `experiments/registry.jsonl` and opening with a *Decision state* section (closed / live / standing evidence). Read that before proposing model work. Settled:
  ...
  - **A zero-shot foundation model beats the incumbent by 20.3% QS / 16% MAE with no features at all** (EXP-021, DM p<0.0001) — but **quote EXP-036 beside it**: against an honest profile floor rather than the single-day carry, the FM's margin halves (+24.8% → +10.7%), because 57% of it was ordinary profile smoothing. Direction unchanged, magnitude roughly half. EXP-022 splits it ~12pp pretrained prior / ~8pp context; EXP-024 shows the context half is not recoverable inside LightGBM — the bottleneck is model class.
  ...
  - **The 20.3% is measured against a weak incumbent (EXP-035, 2026-09-06).** Re-scored against a seasonal-naive floor on the same 260 vintages: Chronos-bolt-base **+25.0%** over naive (DM p<1e-6), above it in **9 of 9 months**; LightGBM `full` only **+5.1%** (p=0.033) and *below* naive in 2 of 9 — including **August 2026, where all eight LightGBM variants are below naive**. The model-class gap is therefore larger than EXP-021 stated. `drop_rolling` is the best LightGBM arm against the floor (+0.107); `drop_calendar` the only one below it overall (−0.017).
...
- Eval: `ml/shadow/eval_log.jsonl` continues to log per-day metrics, **plus a seasonal-naive skill floor from 2026-09-06**
...
- Open: **augur#31** (**FIXED 2026-09-11** — expectation moved median→75th percentile, restoring 192; plus a 4h bounded hold on a short primary so an off-schedule short can still be superseded. Asserting span was found **not implementable**: the EDH report carries no time range and the payloads are encrypted — upstream confirms *"span has no accidental detector"*. Their own span check is still unarmed), **augur#32** (ARF is wall-clock anchored, so it degrades to a 0-hour forecast where production degrades to stale — decide repair vs retirement before any work), **augur#33** (dashboard shows no staleness indicator; `t0` and `last_updated` both exist in the JSON and neither reaches the page), **augur#29** (**point-forecast skill floor** — production does not reliably beat seasonal-naive; edge is +5% over the year but *negative* in the current high-price regime, where all eight LightGBM variants fall below the floor. Verdict pre-committed on ≥21 rows carrying `lightgbm_skill_vs_naive` **and ≥12 paired hours** (floor added 2026-09-11 after a 1-hour row scored +0.762 and counted as a full day; see the hypothesis-log amendment). Count stands at **2/21** and the daily job now prints it, so ≈2026-09-27 assumes an unbroken run from here and is optimistic. **Read this before quoting any relative effect size in this repo**), **augur#30** (the first ~24 forecast hours predict a day-ahead auction that cleared ~4h earlier, and `api-client.js:226` already fetches the published prices — a product change, not yet pre-committed, and blocked on whether EDH's file carries tomorrow), **augur#19** (calibration — now an *upper-side* / band-width gap, see the reframed weakness bullet above; EXP-017's premise stale; EXP-020 confirmed fundamentals do not fix it), **augur#28 / EXP-018a** (feature reduction — awaiting fresh vintages ≈2026-09-09), **augur#15 / EXP-021a** (foundation models — **first arm run 2026-08-29 and it won decisively**: Chronos-Bolt zero-shot −20.3% QS vs lean, raw band coverage 0.811 vs 0.611, nothing shipped pending EXP-021a's fresh-vintage + feasibility + shadow stages), **augur#14** (gap *detection* shipped 2026-08-28; automated backfill still undecided — a reconstructed vintage built from fresher exogenous would not be comparable with the live ones beside it), **augur#25** (event-driven EDH trigger — the 2026-08-24 90-second race is the concrete case for it). Closed and archived: augur#3, #12, #26, #27 — see `memory/session-archive.md`.
...
| `ml/shadow/evaluate_shadow.py` | Daily LGBM-vs-ARF metrics, appends to `ml/shadow/eval_log.jsonl`. **Also scores a seasonal-naive floor since 2026-09-06** (`naive_mae`, `lightgbm_skill_vs_naive`) and, since 2026-09-11, **EXP-036's stronger profile floor beside it** (`naive_profile_mae`, `lightgbm_skill_vs_naive_profile` — the mean of the same clock hour over the last 5 *matching day types*). Additive by pre-commitment: the carry's fields are untouched so pre-2026-09-11 rows stay comparable, and augur#29's verdict is still read against the carry — every other metric in this project is *relative*, so both models being worse than a one-line baseline was invisible to all of them. Baseline may only read `24*ceil(h/24)` hours back; absent parquet leaves the fields null, never 0.0 |
| `ml/shadow/eval_log.jsonl` | Append-only eval log per realised eval day. Rows before 2026-09-06 have no `naive_*` fields and rows before 2026-09-11 no `naive_profile_*` fields — that is age, not a failed baseline. `naive_profile_*` is null when the parquet lacks `is_holiday_nl`: an empty holiday set would silently demote the arm to weekday/weekend matching, a different estimator under the same field name |
...
| `scripts/daily_update.sh` | Sadalsuud daily job — ARF backup + parquet consolidate + LGBM retrain + shadow eval + commit + push. Triggered by `scripts/systemd/augur-daily.timer` (deployed to `/etc/systemd/system/`), gated by `scripts/wait_for_edh.sh` polling EDH freshness. **ARF runs NON-FATAL (`set +e`) since 2026-07-03** — a backup-signal failure can no longer abort the production LightGBM push; its rc shows as `ARF OK`/`ARF FAIL rc=N` in the commit subject. Pre-flight alarms (all surface in the commit subject): `SHADOW_PRE_AGE_H >36h` (stale state), `DEP_PROBE_OK=0` (**lightgbm/pandas** import broken — river deliberately excluded 2026-07-03 so a broken backup-only dep can't skip production), and a non-blocking pytest smoke gate (`SMOKE_OK`, writes `logs/smoke.log`). **Four** post-run output guards: ARF forecast <24h and no new eval row >2 days (2026-06-12, augur#14), **the seasonal-naive floor** (2026-09-06), split in two on review the same day: `[NOTE: sub-naive L/N]` when the model is below the floor on ≥5 of the last 7 *scored* rows — a **result**, kept out of the `ALARM:` path so the heartbeat does not mail "pipeline FAILED" for a non-failure — and `[ALARM: naive unscored N]` when eval rows land carrying no score at all, which **is** a fault (the instrument broke) — plus, since 2026-09-11, a **paired-hour floor** (`MIN_NAIVE_HOURS`, default 12) on both counts, because a row scored on one hour stood as a whole day beside 24-hour rows (`[NOTE: naive thin N]` once 3+ of the last 10 are excluded; the 7-row window walks back until it has 7 *qualifying* rows, since a fixed 10-row scan could be starved by the excluded ones), and a `Seasonal-naive verdict trigger: N/21 qualifying rows` line that makes augur#29's trigger mechanical instead of counted by eye — and **t0 not advancing exactly one calendar day** (2026-08-28) — `[ALARM: t0 stale <date>]` when a stale parquet makes the run overwrite yesterday's vintage, `[ALARM: t0 jumped Nd]` when a day is skipped and becomes permanently unevaluable. |
| `scripts/wait_for_edh.sh` | systemd `ExecStartPre` gate. **Readiness is a CONTENT contract, not a clock window (rewritten 2026-09-01, `7d0066d`)**: the EDH report must be strictly newer than the last consumed (`logs/.edh_gate_state`) **and** the primary dataset `entsoe` must carry at least the **75th-percentile point count of the last N publishes** (`expected_points`, fallback 192) — so an upstream resolution change is still absorbed in a few days, while a run of degraded publishes no longer teaches the gate that short is normal. **Was the median until 2026-09-11 (augur#31): it had decayed 192→96, so the gate could not see a short publish at all.** A short primary is now **held for up to 4h** (`EDH_SHORT_HOLD_HOURS`) and then **accepted with an alarm**, rather than refused to the deadline — six of the last nine shorts were off-schedule catch-ups superseded by a full publish (2026-09-04 went short×3 then 192), while 2026-09-08's scheduled short was never superseded. Neither pure position fits both; the bound costs the second case 4h instead of ~8 and keeps the first case's save. `SHORT_HOLD_HOURS=0` restores pure accept-and-alarm. A short *secondary* feed (`load_forecast`) does **not** block, and since 2026-09-11 rides as `[NOTE: ...]` rather than `[ALARM: ...]` — as an alarm it made the heartbeat mail "pipeline FAILED" for the healthy 2026-09-10 run. The gate sees a point count, not a feature row; whether a short feed costs anything is `latest_feasible_t0`'s call, and that answer arrives as `[ALARM: t0 held back Nh]`. Polls to a 03:00 UTC deadline, then proceeds anyway: never fail-closed, and the t0-advance alarm marks it. **Since 2026-09-10 it can also give up EARLY** by reading EDH's `publish-failure` issue label on `ducroq/energydatahub` (their alert job opens one issue per failed publish and closes it on recovery; read unauthenticated from `api.github.com` with python3 stdlib — **not `gh`, which is not installed on sadalsuud**) — this is the only thing that separates *EDH is late* from *EDH has failed and nothing is coming tonight*, which the deadline cannot. Reference point is **this run's START**, not the last consumed report: an issue from a previous night is routinely still open at 16:30, and comparing against the last publish would abandon the wait before EDH's (GitHub-deferred, 17:50–19:30) run began. Emits `[ALARM: EDH publish FAILED upstream — <repo>#<n>]`, does not record the report as consumed, and every way of not knowing (no `gh`, no auth, no network, rate limit) falls through to the deadline — it can only ever END a wait early, never start or extend one. `EDH_ISSUE_POLL_EVERY=0` disables. The old `MIN_PUBLISH_HOUR_UTC=12` floor is **gone** — GitHub defers EDH's cron unpredictably (observed 00:18–21:14 UTC), so a clock floor shut the gate on good publishes. Earned its place 2026-09-04 by refusing a 06:53 UTC recovery publish whose `entsoe` held same-day prices only (96 vs 192), the day-ahead auction not having cleared. (**augur#31 — that protection lapsed and was restored 2026-09-11**: the median had decayed to 96, confirmed live on the 2026-09-10 07:41 UTC recovery; the 75th percentile returns 192 against the same history.) **Limitation of the failure probe:** EDH opens one issue per failure *streak*, so it fires on the first night of one and not on later nights. ⚠️ It asserts span on `entsoe`/`load_forecast` only and examines no other feed — `wind_speed_80m`/`solar_ghi` feeds are unchecked; see the gotcha-log Promoted table [2026-09-04]. |
...
| `scripts/heartbeat_check.sh` | Daily 06:00 UTC liveness check (`augur-heartbeat.timer`). Covers what `OnFailure=` structurally **cannot**: `daily_update.sh` ends with `git diff --cached --quiet && echo "No changes to commit" \|\| {commit; push;}`, so a run producing nothing exits 0 and the unit *succeeds*; likewise a disabled timer or a down box. Alarms on no `Daily update` commit in >30h, timer not enabled/active, unpushed commits, or an `ALARM:`/non-zero-rc marker in the newest commit subject. **`[NOTE: ...]` markers are invisible to it by design** — that is the whole NOTE/ALARM split, and producers must respect it (the EDH secondary-short marker did not, and mailed a failure for the healthy 2026-09-10 run). Since 2026-09-11 the soft-failure paragraph also distinguishes *a step produced nothing* (an rc marker is present) from *a guard fired on a run that completed* (`rc=0`); it previously asserted the first unconditionally. Exits 0 when it *sends* an alert (its own `OnFailure=` is reserved for a broken watchman). Blind spot, by design: it runs on sadalsuud, so it cannot report sadalsuud being down — that needs an off-host dead-man's switch, i.e. a new service, declined 2026-07-17. |
...
| Path | What it is |
|------|-----------|
| `experiments/registry.jsonl` | Append-only experiment log (EXP-001..EXP-037, gap at EXP-017 which was never run); schema in `experiments/README.md` |
...
| `docs/experiment-backlog.md` | ADR-007 pre-commits. The 2026-08-29 batch (EXP-023..029) is **all run**; the one open entry is **EXP-034** (fragility-conditioned bands, added 2026-08-31, queued after EXP-018a Stage 1); **EXP-036** (profile floor) and **EXP-037** (rank/cost-regret metric) were pre-committed and **both run 2026-09-11**. All run on the existing window and consume no fresh vintages |
...
| `tests/` | pytest suite — **372 tests** (recorded as 243 until 2026-09-04, when a `/curate` verify probe caught the drift) (SecureDataHandler, OnlineFeatureBuilder, LGBM forecaster + multi-horizon + secure_pickle + conformal + backtest + update_shadow + evaluate_shadow + slice MAE + archive path + metrics module + **consolidate parsers (test_consolidate.py, 18 tests, 2026-06-10)** + **t0-advance guard (TestClassifyT0Advance, 7 tests, 2026-08-28)** + **EXP-020 fundamentals parsers (20 tests, 2026-08-29)** + **alert-channel credential layer + exit contract (test_notify_email.py, 12 tests, 2026-08-30)** + **feasible-t0 selection + hold-back state (TestLatestFeasibleT0 + TestHoldBackReachesState, 9 tests, 2026-08-31)** + **seasonal-naive baseline: horizon lookback, t0 resolution, ENTSO-E-only history, paired-hour restriction (25 tests, 2026-09-06)** + **heartbeat notification policy (test_heartbeat_check.py, 16 tests) — marker separation, EDH-stall shape stability, and failed-send retry across an episode boundary (5 added 2026-09-10)** + **EDH upstream-failure probe (TestUpstreamFailureSignal, 10 tests, 2026-09-10)** + **backlog Method-body parser (test_audit_registry.py, 4 tests, 2026-09-10)** + **seasonal-naive floor counting: span-degenerate row exclusion + the mechanical ≥21 trigger (test_naive_floor_guard.py, 22 tests, 2026-09-11)** + **NOTE-vs-ALARM separation and finding-text accuracy in the heartbeat (9 added 2026-09-11)**) |
...
| `scripts/audit_registry.py` | Registry auditor — 5 checks incl. number-traceability and pre-commit immutability. Check 5 resolves each backlog entry to **its own** pre-commit revision via `PRECOMMIT_REV_BY_ID` (2026-08-31); pinning all entries to one global revision made adding a new pre-commitment fail forever against an empty baseline. **A second false-positive of the same family fixed 2026-09-10**: `method_sections` let the *last* section run to EOF, so appending an entry changed the previous one's captured body and reported it as EDITED — the whole diff being the `---` separator. Bodies are now normalised, both sides of the comparison go through the same parser, and existing pins validate unchanged |
...
| `scripts/exp036_profile_floor.py` | EXP-036 — seven baseline estimators against the single-day carry (N1..N7, decay, day-type-matched). Adopted `N5_dt`: the carry was 15.8% worse than an honest floor, which turns the incumbent's +5.1% into −12.4%. Emits the pre-committed Alternative signals into the artifact so no write-up number is computed by hand |
| `scripts/exp037_rank_metric.py` | EXP-037 — cost regret in EUR/MWh (what you pay running a load for the k hours a forecaster calls cheapest, vs perfect foresight). Method pre-committed before any number existed; gates evaluated mechanically in the output. Imports EXP-035's baseline so the carry is bit-identical |
| `scripts/exp037_diagnostics.py` | EXP-037 post-hoc mechanism + artefact check — forecast amplitude by horizon group, and sorted `p50` vs raw tau=0.50. Separate file so the pre-committed runner's numbers stay untouched |
| `scripts/exp035_naive_floor.py` | EXP-035 — re-scores every stored EXP-018/EXP-021 arm against a seasonal-naive floor. No GPU, no fresh vintages; naive prices read from the FM's own context tape so the baseline's information set is a subset of the candidate's *by construction* |
