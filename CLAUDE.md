...
sadalsuud (systemd timer: 16:30 UTC start + wait_for_edh.sh gate, fires after EDH publishes)
    ...
    │           report newer than last consumed AND entsoe >= 75th-pct points of recent
    │           publishes. Deadline 03:00 UTC, then proceeds anyway (never fail-closed).
    │           ⚠️ "newer than last consumed" is MONOTONICITY, NOT FRESHNESS — the unit
    │           fires 2h before EDH publishes, so an unconsumed publish left by any
    │           multi-publish EDH day releases the run instantly and the pipeline then
    │           runs a day behind indefinitely, with every guard green (augur#34).
...
- **Known weakness — no demonstrated skill over naive (augur#29, opened 2026-09-06)**: horizon-matched over `t0 ∈ [2026-07-30, 2026-08-24]`, LGBM `p50` loses to "same clock hour, last day available" at **every** horizon, and wins on only **44%** of 96 mixed-horizon eval days. It was invisible because every instrument here is *relative* — `eval_log` scores LGBM vs ARF, the registry scores variants vs production, DM compares two candidates. **Read every pre-2026-09-06 effect size with this in mind.** Confirmed twice on 2026-09-11 and both times it got worse, not narrower: EXP-037 (cost regret — the *product* lens) and EXP-036 (the floor itself was 15.8% too weak). Numbers and mechanism: `docs/experiment-results.md` Decision state; verdict pre-commitment in `docs/hypothesis-log.md` [2026-09-06].
...
- **Pipeline reliability (2026-08-28)**: t0 is data-derived (`parquet.index.max()`), so a stale parquet used to stall it — overwriting a vintage — and a catching-up parquet used to skip one, permanently. Both now alarm in the daily commit subject via `classify_t0_advance`, and `wait_for_edh.sh` gates on a content contract. ⚠️ **That contract is monotonic, not fresh** — it was a ≥12:00 UTC stamp until 2026-09-01, and the clock floor's removal is what let the gate settle into a one-day lag twice in 2026-09 (augur#34). Residual risk is upstream: EDH published on only 31 of 35 days over 2026-07-25..08-28 (ducroq/energyDataHub#50). Known permanent `eval_log.jsonl` holes: 2026-06-08, 2026-06-10, 2026-08-25, **2026-09-13** (the last one ours, not upstream — augur#34). Position + 14-run review in `docs/hypothesis-log.md` [2026-08-28].
...
- Open (full detail in each GH issue; **read augur#29 before quoting any relative effect size in this repo**): **augur#29** (**point-forecast skill floor — the model loses to a one-line baseline**, and two independent 2026-09-11 checks made that worse rather than narrower: EXP-037 shows it ranks cheap hours *worse* than the carry, EXP-036 shows the carry itself was 15.8% too weak, taking the incumbent to −12.4%. Verdict pre-committed on ≥21 rows with ≥12 paired hours, ≈2026-09-27, currently **5/21** — 4 of those 5 are *above* the carry, which is exactly why EXP-036's profile floor matters: it is live since 2026-09-11 and its first 3 rows read +0.55 / −0.12 / +0.06), **augur#30** (first ~24 forecast hours predict an already-cleared auction — ⚠️ **was recorded here as "deliberate, the zero-latency self-test"; it was neither deliberate nor stable.** It was a property of the augur#34 gate lag, and since 2026-09-13 the block is **gone** — `h=1..24` now covers a day whose auction clears ~16h *after* publication. It returns silently if the lag re-seeds. EXP-037 shows that block flatters, so stratify per horizon before trusting any pre-2026-09-13 score there. Whether the *dashboard* should show published prices where the auction has cleared is still open), **augur#19** (calibration — now an upper-side / band-width gap; EXP-017's premise stale, EXP-020 confirmed fundamentals do not fix it), **augur#15 / EXP-021a** (foundation models — Chronos wins on MAE *and* cost regret in every month, but EXP-036 halves the headline margin: +24.8% over the carry → **+10.7%** over an honest floor. Gated on 14 fresh vintages), **augur#28 / EXP-018a** (feature reduction, fresh vintages), **augur#32** (ARF is wall-clock anchored so it degrades differently from production — repair vs retire undecided), **augur#33** (dashboard has no staleness indicator though `t0` and `last_updated` both exist in the JSON), **augur#14** (gap detection shipped; automated backfill still undecided), **augur#25** (event-driven EDH trigger — now the structural fix for augur#34, not just a latency win), **augur#34** (**the EDH gate has no freshness test**, so it settles into a one-day lag that costs vintages and moved the forecast anchor 24h; the regime decision gates the gate fix, and the two candidates imply *opposite* changes). Closed: augur#3, #12, #26, #27, **#31** — see `memory/session-archive.md`.
...
| `scripts/daily_update.sh` | Sadalsuud daily job — ARF backup + parquet consolidate + LGBM retrain + shadow eval + commit + push. Triggered by `scripts/systemd/augur-daily.timer`, gated by `wait_for_edh.sh`. **ARF runs NON-FATAL since 2026-07-03** so a backup-signal failure cannot abort the production push. **Every alarm this pipeline raises rides in the commit SUBJECT** — that is the channel `heartbeat_check.sh` reads. Pre-flight: stale-state age, dep probe (lightgbm/pandas), pytest smoke. Post-run guards: ARF forecast <24h, eval row >2 days, the seasonal-naive floor, and t0 not advancing exactly one day. **`[ALARM: ...]` = a fault; `[NOTE: ...]` = a result worth recording and deliberately invisible to the heartbeat** — respect that split when adding a marker. Every marker is documented symptom-by-symptom in `docs/RUNBOOK.md`. |
| `scripts/wait_for_edh.sh` | systemd `ExecStartPre` gate. **Readiness is a CONTENT contract, not a clock window**: the EDH report must be strictly newer than the last consumed (`logs/.edh_gate_state`) **and** `entsoe` must carry at least the **75th-percentile point count of recent publishes** (`expected_points`, fallback 192). ⚠️ **Strictly-newer is monotonicity, not freshness** — it stops a publish being used twice; it does not require the publish to be today's. The unit fires ~2h before EDH publishes, so one unconsumed publish (any multi-publish EDH day) makes every later run release on yesterday's data within seconds, indefinitely and invisibly — `classify_t0_advance` checks the *delta*, which a uniformly-lagged pipeline satisfies exactly. Detect with `grep 'READY:' logs/daily_update.log` and compare consumed-vs-run date. **augur#34**; do not fix with a clock floor. A short primary is **held up to 4h then accepted with an alarm** (`EDH_SHORT_HOLD_HOURS`; `0` = accept immediately); a short *secondary* never blocks and rides as `[NOTE: ...]`. Polls to a 03:00 UTC deadline then proceeds — **never fail-closed** — and can give up early on EDH's `publish-failure` issue label (read with python3 stdlib; **`gh` is not installed on sadalsuud**). There is no `MIN_PUBLISH_HOUR_UTC` floor: GitHub defers EDH's cron unpredictably (00:18–21:14 UTC observed). ⚠️ It asserts size on `entsoe`/`load_forecast` only — **span is not assertable**, the report carries no time range and the payloads are encrypted. The script header carries the full reasoning; operational symptoms are in `docs/RUNBOOK.md`; the augur#31 decisions in `docs/hypothesis-log.md` [2026-09-10/11]. |
| `scripts/check_gate_lag.sh` | Read-only answer to *is the gate consuming the current EDH publish, or an older one?* Compares `logs/.edh_gate_state` against the timestamp in EDH's newest committed `data_quality_report.json` — a comparison, not a heuristic. Exists because nothing else can see augur#34: a uniformly-lagged pipeline advances `t0` by exactly +1 day per run, so `classify_t0_advance` reads green throughout. Exits non-zero only when genuinely lagged, so it backs the `<!-- verify: -->` probe on the memory index; every way of not knowing prints `CANNOT VERIFY` and exits 0. Run it over ssh with `bash -s` so it tests the box without needing a pull first. |
...
| `tests/` | pytest suite — **372 tests** (recorded as 243 until 2026-09-04, when a `/curate` verify probe caught the drift). Covers SecureDataHandler, feature builders, the LGBM forecaster + multi-horizon + secure_pickle + conformal + backtest, `update_shadow`, `evaluate_shadow` (incl. the seasonal-naive floor and EXP-036's profile floor), `consolidate` parsers, the t0-advance and feasible-t0 guards, the metrics module, and the three shell guards — `wait_for_edh.sh`, `heartbeat_check.sh`, `notify_email.py` — plus the registry auditor and the naive-floor counting guard. New test files are named for what they pin; `git log --stat tests/` dates each group. |
...
| `ml/shadow/exp018_stage0*/summary.json`, `ml/shadow/exp019_stationary/summary.json`, `ml/shadow/exp020_fundamentals{,_ctl}/summary.json`, `ml/shadow/exp021_foundation{,_aligned}/summary.json`, `ml/shadow/exp022_context_ladder/summary.json`, `ml/shadow/exp036_profile_floor/summary.json`, `ml/shadow/exp037_rank_metric/summary.json` | Sweep records (per-hour `predictions.parquet` / FM context dumps are gitignored, regenerable) |
