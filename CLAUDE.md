...
- Open (full detail in each GH issue; **read augur#29 before quoting any relative effect size in this repo**): **augur#29** (**point-forecast skill floor — the model loses to a one-line baseline**, and two independent 2026-09-11 checks made that worse rather than narrower: EXP-037 shows it ranks cheap hours *worse* than the carry, EXP-036 shows the carry itself was 15.8% too weak, taking the incumbent to −12.4%. Verdict pre-committed on ≥21 rows with ≥12 paired hours, ≈2026-09-27, currently **2/21**), **augur#30** (first ~24 forecast hours predict an already-cleared auction — **deliberate, it is the zero-latency self-test**; but EXP-037 shows that block flatters, so stratify per horizon before trusting it. Whether the *dashboard* should show published prices there is still open), **augur#19** (calibration — now an upper-side / band-width gap; EXP-017's premise stale, EXP-020 confirmed fundamentals do not fix it), **augur#15 / EXP-021a** (foundation models — Chronos wins on MAE *and* cost regret in every month, but EXP-036 halves the headline margin: +24.8% over the carry → **+10.7%** over an honest floor. Gated on 14 fresh vintages), **augur#28 / EXP-018a** (feature reduction, fresh vintages), **augur#32** (ARF is wall-clock anchored so it degrades differently from production — repair vs retire undecided), **augur#33** (dashboard has no staleness indicator though `t0` and `last_updated` both exist in the JSON), **augur#14** (gap detection shipped; automated backfill still undecided), **augur#25** (event-driven EDH trigger). Closed: augur#3, #12, #26, #27, **#31** — see `memory/session-archive.md`.
