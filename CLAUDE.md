...
sadalsuud (systemd timer: 16:30 UTC start + wait_for_edh.sh gate, fires after EDH publishes)
    ...
    │           (a) report newer than last consumed, (b) entsoe >= 75th-pct points of
    │           recent publishes, and (c) since 2026-09-14 the report must have arrived
    │           WHILE WE WERE WAITING — (a) is monotonicity, not freshness, and without
    │           (c) an unconsumed publish left by any multi-publish EDH day released the
    │           run instantly and the pipeline ran a day behind indefinitely with every
    │           guard green (augur#34). (c) holds, never refuses, and is relative to the
    │           run, so it is NOT the clock floor removed 2026-09-01.
    │           Deadline 03:00 UTC, then proceeds anyway (never fail-closed).
...
- Open (detail in each GH issue). ⚠️ **Read augur#29 before quoting any relative effect size in this repo** — the production point forecast loses to a one-line baseline, and two independent 2026-09-11 checks widened that rather than narrowing it: EXP-037 (it ranks cheap hours *worse* than the carry) and EXP-036 (the carry itself was 15.8% too weak, taking the incumbent to **−12.4%**). Verdict pre-committed on ≥21 rows with ≥12 paired hours, ≈2026-09-27, at **5/21** — 4 of those 5 are *above* the carry, which is why EXP-036's profile floor matters (live since 2026-09-11; first 3 rows +0.55 / −0.12 / +0.06). **augur#30** (first ~24 forecast hours — ⚠️ **recorded here until 2026-09-14 as a deliberate "zero-latency self-test"; it was neither deliberate nor stable.** It was the augur#34 gate lag, and since 2026-09-13 the block is **gone**: `h=1..24` now covers a day whose auction clears ~16h *after* publication. Stratify per horizon before trusting any pre-2026-09-13 score there. The dashboard half is still open), **augur#19** (calibration — upper-side / band-width gap; ⚠️ new confound since the 2026-09-13 anchor shift, see augur#34), **augur#15 / EXP-021a** (foundation models — Chronos wins on MAE *and* cost regret every month, but EXP-036 halves the headline: +24.8% over the carry → **+10.7%** over an honest floor; gated on fresh vintages), **augur#28 / EXP-018a** (feature reduction — **13/14 vintages**, gate met on the next clean run), **augur#32** (ARF wall-clock anchored; repair vs retire undecided), **augur#33** (no dashboard staleness indicator), **augur#14** (gap detection shipped; backfill undecided), **augur#25** (event-driven EDH trigger — the structural fix for augur#34, not just a latency win). Closed: augur#3, #12, #26, #27, #31, **#34** (gate freshness + anchor regime, 2026-09-14) — see `memory/session-archive.md`.
...
| `scripts/wait_for_edh.sh` | systemd `ExecStartPre` gate. **Readiness is a CONTENT contract, not a clock window**: the EDH report must be strictly newer than the last consumed (`logs/.edh_gate_state`) **and** `entsoe` must carry at least the **75th-percentile point count of recent publishes** (`expected_points`, fallback 192). **and (c)** the report must have arrived while this run was *waiting* (`STALE_GRACE_HOURS`, default 2h) — strictly-newer is **monotonicity, not freshness**, and without (c) the pipeline silently ran a day behind (augur#34). (c) holds rather than refuses, and is relative to the run, not the clock. A short primary is **held up to 4h then accepted with an alarm** (`EDH_SHORT_HOLD_HOURS`; `0` = accept immediately); a short *secondary* never blocks and rides as `[NOTE: ...]`. Polls to a 03:00 UTC deadline then proceeds — **never fail-closed** — and gives up early on EDH's `publish-failure` label (python3 stdlib; **`gh` is not on sadalsuud**). No `MIN_PUBLISH_HOUR_UTC` floor. ⚠️ Size only on `entsoe`/`load_forecast` — **span is not assertable** (no time range in the report, payloads encrypted). **The script header carries the full reasoning for every clause — read it before changing any of them**; symptoms in `docs/RUNBOOK.md`; decisions in `docs/hypothesis-log.md` [2026-09-10/11] and [2026-09-14]. |
