...
| `ml/shadow/evaluate_shadow.py` | Daily LGBM-vs-ARF metrics, appends to `ml/shadow/eval_log.jsonl`. **Also scores a seasonal-naive floor since 2026-09-06** (`naive_mae`, `lightgbm_skill_vs_naive`) — every other metric in this project is *relative*, so both models being worse than a one-line baseline was invisible to all of them. Baseline may only read `24*ceil(h/24)` hours back; absent parquet leaves the fields null, never 0.0 |
| `ml/shadow/eval_log.jsonl` | Append-only eval log per realised eval day. Rows before 2026-09-06 have no `naive_*` fields — that is age, not a failed baseline |
...
| `docs/experiment-backlog.md` | ADR-007 pre-commits. The 2026-08-29 batch (EXP-023..029) is **all run**; the open entries are **EXP-034** (fragility-conditioned bands, added 2026-08-31, queued after EXP-018a Stage 1) and **EXP-036** (profile baseline as a stronger skill floor, added 2026-09-10); **EXP-037** (rank/cost-regret metric, 2026-09-11) is pre-committed and **run**. All run on the existing window and consume no fresh vintages |
