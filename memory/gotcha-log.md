...
**Residual risks (surfaced, user decision pending)**: locked `pandas==3.0.4` is a *yanked* release (reported datetime segfaults) — a datetime-heavy TS pipeline should prefer a stable non-yanked pin; and dev/prod Python skew (situla 3.14 vs sadalsuud 3.12, situla has no 3.12). Both argue for pinning a conservative, mutually-tested stack on a matched interpreter.
**Status**: [RESOLVED for the outage] — production restored, reproducible + decoupled + smoke-gated. Residual pin/interpreter hardening and alerting are follow-ups.
