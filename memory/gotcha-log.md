...
**Followup**: Discovered in the same investigation but a separate live issue: ARF forecasts are truncated to 48h (was 72h) since the v2.2 fix, because post-v2.2 `load_forecast`/`energy_price_forecast` files only span today+tomorrow (~48h at 15-min) vs ~8 days pre-v2.2. Not a parser bug — the data is gone from the files. Filed as **augur#26** (EDH-side window restore preferred). Also noted there: the 06-11 run reported "5 hours with exchange lags" when ~25h of day-ahead prices were available — unexplained, check the ENTSO-E contamination guard in `parse_price_file` when working `ml/update.py`.
**Status**: [RESOLVED] for the guards + forensics; truncation tracked OPEN at augur#26.
