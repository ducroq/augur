...
| EDH schema bumps need consumer-side audit on the non-Python side: Python migrates transparently via `_migrate_X_to_Y` in `utils/schema_registry.py`, JS reads raw JSON and silently emits zero traces if the file shape changes. Grep `static/js/` for bracket-access on raw file refs before assuming everything migrated. | User-memory `feedback_edh_schema_consumer_audit.md` (cross-conversation) | 2026-06-07 |
...
**Pattern**: When `energydatahub` bumps schema (anything affecting `to_dict()` envelope shape or schema_version), Python consumers migrate transparently via `_migrate_X_to_Y` in `utils/schema_registry.py`, but **dashboard JS does NOT** — it reads raw JSON. Three places to audit on any EDH schema-touching commit: `static/js/modules/data-processor.js` (wholesale/forecast loop), `static/js/modules/tab-charts.js` (per-tab processors), `static/js/dashboard.js` (`lastUpdate` and any `this.energyData?.X` access). Symptom is silent — chart renders fewer traces or none, no error message. Same diagnostic flow as 2026-06-06's "Browser cache" gotcha (live JS via curl) plus comparing the CDN file shape against what the JS expects.
**Status**: [RESOLVED] — `4a557c8` deployed; user verified. UX hardening backlog at augur#24 (5 findings RPN 120-288, no FAIL). Memory updated: `feedback_edh_schema_consumer_audit.md`.
