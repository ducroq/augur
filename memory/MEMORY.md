...
- Test suite: 202 tests passing (177 + 18 in `test_consolidate.py` + 7 in `TestClassifyT0Advance`, 2026-08-28, for the t0-advance guard) (was 195 + 18 new in `test_consolidate.py` for EDH v2.1/v2.2 parser shapes, units, timezone normalisation, isinstance guards — added 2026-06-10 as code-review followup to the v2.2 fix).
<!-- verify: n=$(.venv/bin/python -m pytest tests/ --collect-only -q 2>&1 | grep -oE "[0-9]+ tests" | grep -oE "[0-9]+"); echo "collected ${n:-none}"; [ -n "$n" ] && [ "$n" -ge 202 ] || exit 1 -->
- Experiment registry: EXP-001..EXP-019 in `experiments/registry.jsonl` (gap at EXP-017 — queued, never run). EXP-014 is the production-promotion entry; EXP-015/016 the parked calibration replays (2026-06-12); EXP-018 the feature ablation (`kept`, evidence); EXP-019 the refuted stationary reparameterisation (`rejected`).
<!-- verify: [ "$(wc -l < experiments/registry.jsonl)" -ge 18 ] && echo PASS || echo FAIL -->
...
- **augur#22** (filed 2026-06-05): HDD/CDD as long-horizon demand features. **Demoted 2026-08-25** behind EXP-018a — EXP-018 measured the three exogenous series already in the model as inert (±0.4% MAE each), so adding a fourth is a weak bet until the model demonstrably uses what it has.
...
- **Deferred ML features** (#2 NED production, #3 gas/carbon, #4 cross-border flows, #22 HDD/CDD): all **demoted 2026-08-25** behind the EXP-018a reduction bet, per EXP-018's inert-exogenous finding. Parsers for #3 remain banked on `feat/new-features-ttf-genmix`. Revisit once the lean feature set is settled and the model shows it can extract value from exogenous inputs.
