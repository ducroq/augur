...
- **augur#26** (filed 2026-06-12): ARF forecast truncated to 48h since EDH v2.2 — `load_forecast`/`energy_price_forecast` files now span only today+tomorrow (~48h @ 15-min) vs ~8 days pre-v2.2. Not a parser bug; EDH-side window restore preferred. Loose end noted there: 06-11 run had only "5 hours with exchange lags" when ~25h were available (check `parse_price_file` contamination guard).
