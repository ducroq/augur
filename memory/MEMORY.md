...
- **EDH v2.2 envelope shim** (`4a557c8`, 2026-06-07): `data-processor.js:117` + `tab-charts.js:64-79` + `dashboard.js:264` now defensively unwrap `obj.data ?? obj` to read the canonical `{metadata, data: {...}}` envelope EDH started emitting for `energy_price_forecast.json` + `wind_forecast.json` (EDH commit `3dfc7fb`). Before the shim, the Prices-tab wholesale lines and Forecast→Wind chart silently rendered zero traces. Pattern promoted to gotcha-log Promoted table + user-memory `feedback_edh_schema_consumer_audit.md`.
<!-- verify: cd /c/local_dev/augur && [ -f static/data/augur_forecast.json ] && [ -f static/data/augur_forecast_shadow.json ] && [ -f layouts/index.html ] && grep -q "utcToLocalNaiveISO" static/js/modules/chart-renderer.js && grep -q "weather-location-cloud" static/js/dashboard.js && grep -q "energyData.data ?? energyData" static/js/modules/data-processor.js && echo PASS || echo FAIL -->
...
- **augur#12** (Phase 1 deployed 2026-06-08, observation in progress): systemd user-timer `augur-daily.timer` at 16:30 UTC + `ExecStartPre=wait_for_edh.sh` polling EDH's `data_quality_report.json:timestamp` until today's date appears (max-wait 4h, exits 0 on timeout so absent-healthchecks ping remains the alarm). Empirically EDH publishes 16:23-20:13 UTC (14-day sample, **not** the 15:20 UTC the old MEMORY claimed). Artifacts canonical at `scripts/wait_for_edh.sh` + `scripts/systemd/{augur-daily.service,augur-daily.timer,README.md}`; sadalsuud's `~/.config/systemd/user/` entries are symlinks. Cron stays active for 1-week observation window (per plan). **Pending**: (a) `sudo loginctl enable-linger jeroen` — user-driven (sudo password needed); without this the timer only fires while jeroen has an active session, currently true but fragile. (b) After first successful 16:30 UTC fire (2026-06-08 evening), comment the cron line in `crontab -e`. (c) After 7 days of clean timer fires, delete the commented cron line. Rollback: `systemctl --user disable --now augur-daily.timer` + uncomment cron.
<!-- verify: ssh sadalsuud "systemctl --user is-enabled augur-daily.timer && systemctl --user is-active augur-daily.timer" 2>/dev/null | grep -q "enabled" && echo PASS || echo FAIL -->
...
- **EDH-side follow-ups discovered today** (not yet filed in energydatahub): (a) `collectors/elspot.py:129-133` only fetches `datetime.now(Amsterdam)` — never asks for tomorrow's prices even though Nord Pool publishes them after 13:00 CET; chart's Elspot trace always stops at end-of-today. (b) pynordpool's `NL` area appears to syndicate ENTSO-E NL day-ahead data (bit-identical values; not the genuinely-different EPEX feed) — Elspot may effectively be redundant since Feb 2026 `40632f6` pynordpool migration. Possible action: file as energydatahub issue, OR remove `elspot` from `static/js/modules/constants.js:49` DATA_SOURCES.forecast.
