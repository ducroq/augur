"""
Mini warmup for the long-history experiment.

Functionally identical to ml/training/warmup.py but writes to a parallel
model directory (ml/models/river_v2/) so the production model is untouched.
Intended for the 1-day mini-check described in
docs/long-history-implementation-plan.md.
"""
...
DEFAULT_MODEL_DIR = Path(__file__).parent.parent / "models" / "river_v2"
...
def warmup(data_path: Path, model_dir: Path):
    ...
    logger.info("=" * 60)
    logger.info(f"Mini warmup complete:")
...
def main():
    ...
    args = p.parse_args()
    warmup(Path(args.data), Path(args.model_dir))
